...
const checkAvailability = async (req, res) => {
  try {
    ...
    if (
      ...
      appointmentTimeFormatted > endTime
    ) {
      return res
        .status(201)
        .json({
    ...
    return res
      .status(200)
      .json({ success: true, message: "Appointment slot is available." });
  } catch (error) {
...
const approveAppoinment = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: "mediconnect.desk@gmail.com", // Sender's email
      to: usr.email, // The email of the user who booked the appointment
      subject: "Appointment Approved",
      html: `<p>Dear ${usr.name},</p>
             <p>Your appointment with Dr. ${appointment.doctorInfo.name} on the date ${appointment.date} at ${appointment.time} has been approved.</p>
             <p>Thank you for choosing our service!</p>`, // You can customize the email content
    };
    ...
    return res.status(200).send({
      success: true,
      message: "Successfully Approved and email notification sent!",
    });
  } catch (error) {
    return res.status(500).send({
...
const rejectAppoinment = async (req, res) => {
  try {
    ...
    // Email options for rejection
    const mailOptions = {
      from: "mediconnect.desk@gmail.com", // Sender's email
      to: usr.email, // The email of the user who booked the appointment
      subject: "Appointment Rejected",
      html: `<p>Dear ${usr.name},</p>
             <p>We regret to inform you that your appointment with Dr. ${appointment.doctorInfo.name} on ${appointment.date} at ${appointment.time} has been rejected.</p>
             <p>If you have any questions or wish to reschedule, please contact us.</p>
             <p>Thank you for your understanding.</p>`, // Customize email content
    };
    ...
    res.status(200).send({
      success: true,
      message: "Successfully Rejected and email notification sent!",
    });
  } catch (error) {
    res.status(500).send({
